...
shinyUI(
  fluidPage(
    ...
    titlePanel(
      title = h3("BRAIN INFORMATICS USING NEURAL NETWORKS", align="center"),
      windowTitle = "BRAIN INFORMATICS"
    ),
    
    sidebarLayout(
      sidebarPanel(
        fileInput("file", "Upload the training Dataset", accept = c(".csv")),
        br(),
        h4(helpText("Check below if the training dataset contains headers")),
        ...
        conditionalPanel(condition = "input.f14 == TRUE",
                         h4(helpText("Enter the values of 14-channel EEG Signals for testing (Default values entered):-")),
